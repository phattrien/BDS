...
<div id="module_show_list">
	...
	<div class="table-responsive">
		<table class="table table-striped table-bordered table-hover">
			...
			<tbody>
				<!-- BEGIN: loop -->
				<tr>
					<td><input type="checkbox" name="nhadatid" value="{ROW.id}"/></td>
	...
	<!-- END: data -->
	<!-- BEGIN: empty -->
	<div class="alert alert-warning">{LANG.topic_nonhadat}</div>
	<!-- END: empty -->
</div>
...
$('a.delete').click(function() {
	var list = [];
	$('input[name=nhadatid]:checked').each(function() {
		list.push($(this).val());
	});
	if (list.length < 1) {
	...
	if (confirm('{LANG.topic_delete_confirm}')) {
		$.ajax({
			type : 'POST',
			url : 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=topicdelnhadat',
			data : 'list=' + list,
			success : function(data) {
				alert(data);
				window.location = 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=topicsnhadat&topicid={TOPICID}';
			}
		});
	}
	return false;
