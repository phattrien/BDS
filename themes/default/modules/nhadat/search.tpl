	...
	<div class="panel panel-default">
		<div class="panel-body">
			...
			<div class="form-group">
				<div class="col-md-4"> {LANG.key_title} (<strong>*</strong>) </div>
	  			<div class="col-md-8"><input type="text" name="q" value="{KEY}" class="form-control" id="key"/></div>
			</div>
			
			<div class="form-group">
				<div class="col-md-4">  {LANG.type_title} (<strong>*</strong>)</div>
	  			<div class="col-md-8">
					<select name="choose" id="choose" class ="form-control">
						<!--option value="" {CHECK1}>{LANG.find_all} </option-->
						...
						<option value="4" {CHECK4}>{LANG.gia} </option>
						<option value="5" {CHECK5}>{LANG.huong} </option>
						<option value="8" {CHECK8}>{LANG.vitri} </option>
						<option value="6" {CHECK6}>{LANG.dientich} </option>
						<option value="7" {CHECK7}>{LANG.lienhe} </option>
			...
			<div class="form-group">
				<div class="col-md-4">{LANG.search_cat} (<strong>*</strong>)</div>
	  			<div class="col-md-8">
...
<div class="panel panel-default">
	<div class="panel-body">
		<h3 class="text-center"><em class="fa fa-filter">&nbsp;</em><strong>{NUMRECORD}</strong> {LANG.search_on} {TITLE_MOD}</h3><hr />
		<!-- BEGIN: noneresult -->
		<p><em>{LANG.search_none} : <strong class="label label-info">{KEY}</strong> {LANG.search_in_module} <strong>{INMOD}</strong></em></p>
		<!-- END: noneresult -->
		
		<!-- BEGIN: result -->
		<h3><a title="{TITLEROW}" href="{LINK}">{TITLEROW}</a></h3>
		<div class="text-justify">
			<p>
				<!-- BEGIN: result_img -->
				<img src="{IMG_SRC}" border="0" width="{IMG_WIDTH}px" class="img-thumbnail pull-left" style="margin: 0 5px 5px 0" />
				<!-- END: result_img -->
			<strong>{LANG.matin} ({ID})</strong>	{CONTENT}  
			</p><hr>
			<div class="content-search">
<strong>{LANG.gia}:</strong> {GIA} {DVT}  
<strong>{LANG.dientich}:</strong> {DIENTICH}{LANG.m2}
<strong>{LANG.vitri}:</strong> {VITRI}
<strong>{LANG.huong}:</strong> {HUONG}  <br><!--
<strong>{LANG.lienhe}:</strong> {LIENHE} 
<strong>{LANG.tel}:</strong> {TEL} 
<strong>{LANG.diachi}:</strong> {DIACHI}-->
<strong>{LANG.pubtime}:</strong> {TIME}
