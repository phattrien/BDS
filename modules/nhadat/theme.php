 ...
 */
 // Bat dang chinh sua
 function viewcat_list_new( $array_catpage, $array_cat_other, $generate_page )
{
	...
	foreach( $array_catpage as $array_row_i )
	{
		...
		foreach( $array_row_i['listcatid'] as $listcatid )
		{
			...
			(($n2 < $num_cat) ? $xtpl->parse( 'main.viewcatloop.cat2.comma2' ) : '');
			$xtpl->parse( 'main.viewcatloop.cat2' );
			++$n2;
		}
			...
			foreach( $array_cat[$key]['content'] as $array_row_i )
			{
				...
				else
				{
					...
					if( $array_row_i['imghome'] != "" )
					{
						$xtpl->assign( 'HOMEIMG', $array_row_i['imghome'] );
						$xtpl->assign( 'HOMEIMGALT', ! empty( $array_row_i['homeimgalt'] ) ? $array_row_i['homeimgalt'] : $array_row_i['title'] );
						$xtpl->parse( 'main.listcat.related.loop.image' );
...
function detail_theme( $nhadat_contents, $array_keyword, $related_new_array, $related_array, $topic_array )
{
	...
	$nhadat_contents['addtime'] = nv_date( "d/m/Y h:i:s", $nhadat_contents['addtime'] );

	$xtpl->assign( 'NHADATID', $nhadat_contents['id'] );
	$xtpl->assign( 'NEWSCHECKSS', $nhadat_contents['nhadatcheckss'] );
	$xtpl->assign( 'DETAIL', $nhadat_contents );
	$xtpl->assign( 'SELFURL', $client_info['selfurl'] );

	if( $nhadat_contents['allowed_send'] == 1 )
	{
		$xtpl->assign( 'URL_SENDMAIL', $nhadat_contents['url_sendmail'] );
		$xtpl->parse( 'main.allowed_send' );
	}

	if( $nhadat_contents['allowed_print'] == 1 )
	{
		$xtpl->assign( 'URL_PRINT', $nhadat_contents['url_print'] );
		$xtpl->parse( 'main.allowed_print' );
	}

	if( $nhadat_contents['allowed_save'] == 1 )
	{
		$xtpl->assign( 'URL_SAVEFILE', $nhadat_contents['url_savefile'] );
		$xtpl->parse( 'main.allowed_save' );
	}

	if( $nhadat_contents['allowed_rating'] == 1 )
	{
		$xtpl->assign( 'LANGSTAR', $nhadat_contents['langstar'] );
		$xtpl->assign( 'STRINGRATING', $nhadat_contents['stringrating'] );
		$xtpl->assign( 'NUMBERRATING', $nhadat_contents['numberrating'] );

		if( $nhadat_contents['disablerating'] == 1 )
		{
			$xtpl->parse( 'main.allowed_rating.disablerating' );
		}

		if( $nhadat_contents['numberrating'] >= $module_config[$module_name]['allowed_rating_point'] )
		{
	...
	if( $nhadat_contents['showhometext'] )
	{
		if( ! empty( $nhadat_contents['image']['src'] ) )
		{
			if( $nhadat_contents['image']['position'] == 1 )
			{
				$xtpl->parse( 'main.showhometext.imgthumb' );
			}
			elseif( $nhadat_contents['image']['position'] == 2 )
			{
	...
	if( ! empty( $nhadat_contents['post_name'] ) )
	{
		$xtpl->parse( 'main.post_name' );
	}

	if( ! empty( $nhadat_contents['author'] ) or ! empty( $nhadat_contents['source'] ) )
	{
		if( ! empty( $nhadat_contents['author'] ) )
		{
			$xtpl->parse( 'main.author.name' );
		}

		if( ! empty( $nhadat_contents['source'] ) )
		{
	...
	// contact lien he
if( ! empty( $nhadat_contents['lienhe'] )  )

	{
		if( ! empty( $nhadat_contents['lienhe'] ) )
		{
			$xtpl->parse( 'main.lienhe.name' );
		}
			$xtpl->parse( 'main.lienhe' );
	}
	
if( ! empty( $nhadat_contents['diachi'] )  )

	{
		if( ! empty( $nhadat_contents['diachi'] ) )
		{
			$xtpl->parse( 'main.diachi.name' );
		}
			$xtpl->parse( 'main.diachi' );
	}
if( ! empty( $nhadat_contents['tel'] )  )

	{
		if( ! empty( $nhadat_contents['tel'] ) )
		{
			$xtpl->parse( 'main.tel.name' );
		}
			$xtpl->parse( 'main.tel' );
	}
if( ! empty( $nhadat_contents['gia'] )  )

	{
		if( ! empty( $nhadat_contents['gia'] ) )
		{
			$xtpl->parse( 'main.gia.name' );
		}
			$xtpl->parse( 'main.gia' );
	}	
if( ! empty( $nhadat_contents['dvt'] )  )

	{
		if( ! empty( $nhadat_contents['dvt'] ) )
		{
			$xtpl->parse( 'main.dvt.name' );
		}
			$xtpl->parse( 'main.dvt' );
	}	
	
if( ! empty( $nhadat_contents['huong'] )  )

	{
		if( ! empty( $nhadat_contents['huong'] ) )
		{
			$xtpl->parse( 'main.huong.name' );
		}
			$xtpl->parse( 'main.huong' );
	}
if( ! empty( $nhadat_contents['ghichu'] )  )

	{
		if( ! empty( $nhadat_contents['ghichu'] ) )
		{
			$xtpl->parse( 'main.ghichu.name' );
		}
			$xtpl->parse( 'main.ghichu' );
	}
if( ! empty( $nhadat_contents['dientich'] )  )

	{
		if( ! empty( $nhadat_contents['dientich'] ) )
		{
			$xtpl->parse( 'main.dientich.name' );
		}
			$xtpl->parse( 'main.dientich' );
	}
if( ! empty( $nhadat_contents['vitri'] )  )

	{
		if( ! empty( $nhadat_contents['vitri'] ) )
		{
			$xtpl->parse( 'main.vitri.name' );
		}
			$xtpl->parse( 'main.vitri' );
	}
	// Het contact lien he
	
	
	if( $nhadat_contents['copyright'] == 1 )
	{
	...
	if( defined( 'NV_IS_MODADMIN' ) )
	{
		$xtpl->assign( 'ADMINLINK', nv_link_edit_page( $nhadat_contents['id'] ) . " " . nv_link_delete_page( $nhadat_contents['id'] ) );
...
function search_result_theme( $key, $numRecord, $per_pages, $page, $array_content, $catid )
{
	...
	if( ! empty( $array_content ) )
	{
		foreach( $array_content as $value )
		{
			...
			$xtpl->assign( 'GIA', BoldKeywordInStr( $value['gia'], $key ) );
			$xtpl->assign( 'HUONG', BoldKeywordInStr( $value['huong'], $key ) );
			$xtpl->assign( 'VITRI', BoldKeywordInStr( $value['vitri'], $key ) );
			$xtpl->assign( 'DIENTICH', BoldKeywordInStr( $value['dientich'], $key ) );
			$xtpl->assign( 'LIENHE', BoldKeywordInStr( $value['lienhe'], $key ) );
			$xtpl->assign( 'DIACHI', BoldKeywordInStr( $value['diachi'], $key ) );
			$xtpl->assign( 'TEL', BoldKeywordInStr( $value['tel'], $key ) );
			$xtpl->assign( 'DVT', BoldKeywordInStr( $value['dvt'], $key ) );
			$xtpl->assign( 'GHICHU', BoldKeywordInStr( $value['ghichu'], $key ) );
			$xtpl->assign( 'ID', BoldKeywordInStr( $value['id'], $key ) );
