...
function nv_del_content_module( $id )
{
	...
	if( $id > 0 )
	{
		...
		if( $number_no_del == 0 )
		{
			...
			$db->query( 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_tags SET numnhadat = numnhadat-1 WHERE tid IN (SELECT tid FROM ' . NV_PREFIXLANG . '_' . $module_data . '_tags_id WHERE id=' . $id . ')' );
...
/**
 * nv_nhadat_get_bodytext()
 *
 * @param mixed $bodytext
 * @return
 */
function nv_nhadat_get_bodytext( $bodytext )
{
	// Get image tags
