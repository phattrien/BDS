...
if( $num_items )
{
	...
	$db->select( 'r.id, r.title, r.alias, r.catid, r.hometext, c.bodytext, r.lienhe, r.dientich, r.diachi, r.tel, r.gia, r.dvt, r.huong, r.vitri, r.ghichu' )
		->limit( $limit )
		->offset( ( $page - 1 ) * $limit );
	$result = $db->query( $db->sql() );
	while( list( $id, $tilterow, $alias, $catid, $hometext, $bodytext,  $lienhe, $dientich, $diachi, $tel, $gia, $dvt, $huong, $vitri, $ghichu ) = $result->fetch( 3 ) )
	{
		...
		$result_array[] = array(
			...
			'dientich' => BoldKeywordInStr( $dientich, $key, $logic ),
			'gia' => BoldKeywordInStr( $gia, $key, $logic ),
			'huong' => BoldKeywordInStr( $huong, $key, $logic ),
