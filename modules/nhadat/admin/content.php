...
if( $nv_Request->get_int( 'save', 'post' ) == 1 )
{
	...
	$rowcontent['lienhe'] = $nv_Request->get_title( 'lienhe', 'post',  'post', '', 1 );
	$rowcontent['diachi'] = $nv_Request->get_title( 'diachi', 'post',  'post', '', 1 );
	$rowcontent['tel'] = $nv_Request->get_title( 'tel', 'post',  'post', '', 1 );
	$rowcontent['gia'] = $nv_Request->get_title( 'gia', 'post',  'post', '', 1 );
	$rowcontent['dvt'] = $nv_Request->get_title( 'dvt', 'post',  'post', '', 1 );
	$rowcontent['huong'] = $nv_Request->get_title( 'huong', 'post',  'post', '', 1 );
	$rowcontent['vitri'] = $nv_Request->get_title( 'vitri', 'post',  'post', '', 1 );
	$rowcontent['dientich'] = $nv_Request->get_title( 'dientich', 'post',  'post', '', 1 );
	$rowcontent['ghichu'] = $nv_Request->get_title( 'ghichu', 'post',  'post', '', 1 );
	...
	if( empty( $error ) )
	{
		...
		$rowcontent['bodytext'] = nv_nhadat_get_bodytext( $rowcontent['bodyhtml'] );
		...
		if( ! empty( $rowcontent['topictext'] ) and empty( $rowcontent['topicid'] ) )
		{
			...
			$_sql = "INSERT INTO " . NV_PREFIXLANG . "_" . $module_data . "_topics (title, alias, lienhe, diachi, tel, gia, huong, vitri, ghichu, dientich, description, image, weight, keywords, add_time, edit_time) VALUES ( :title, :alias, :description, '', :weight, :keywords, " . NV_CURRENTTIME . ", " . NV_CURRENTTIME . ")";
			$data_insert = array();
			$data_insert['title'] = $rowcontent['topictext'];
			$data_insert['alias'] = $aliastopic;
			$data_insert['lienhe'] = $rowcontent['lienhe'];
			$data_insert['diachi'] = $rowcontent['diachi'];
			$data_insert['tel'] = $rowcontent['tel'];
			$data_insert['gia'] = $rowcontent['gia'];
			$data_insert['dvt'] = $rowcontent['dvt'];
			$data_insert['huong'] = $rowcontent['huong'];
			$data_insert['vitri'] = $rowcontent['vitri'];
			$data_insert['dientich'] = $rowcontent['dientich'];
			$data_insert['ghichu'] = $rowcontent['ghichu'];
		...
		if( $rowcontent['id'] == 0 )
		{
			...
			$sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_rows
				(catid, listcatid, topicid, admin_id, author, sourceid, addtime, edittime, status, publtime, exptime, archive, title, alias, lienhe, diachi, tel, gia, dvt, huong, vitri, dientich, ghichu, hometext, homeimgfile, homeimgalt, homeimgthumb, inhome, allowed_comm, allowed_rating, hitstotal, hitscm, total_rating, click_rating) VALUES
		...
		else
		{
			...
			$sth->bindParam( ':lienhe', $rowcontent['lienhe'], PDO::PARAM_STR );			
			$sth->bindParam( ':diachi', $rowcontent['diachi'], PDO::PARAM_STR );			
			$sth->bindParam( ':tel', $rowcontent['tel'], PDO::PARAM_STR );			
			$sth->bindParam( ':gia', $rowcontent['gia'], PDO::PARAM_STR );
			$sth->bindParam( ':dvt', $rowcontent['dvt'], PDO::PARAM_STR );			
			$sth->bindParam( ':huong', $rowcontent['huong'], PDO::PARAM_STR );		
			$sth->bindParam( ':vitri', $rowcontent['vitri'], PDO::PARAM_STR );		
			$sth->bindParam( ':dientich', $rowcontent['dientich'], PDO::PARAM_STR );		
			$sth->bindParam( ':ghichu', $rowcontent['ghichu'], PDO::PARAM_STR );	 
		...
		if( empty( $error ) )
		{
			...
			foreach( $id_block_content_del as $bid_i )
			{
				...
				nv_nhadat_fix_block( $bid_i, false );
			}
				...
				foreach( $keywords as $keyword )
				{
					if( ! in_array( $keyword, $array_keywords_old ) )
					{
						...
						if( empty( $tid ) )
						{
							...
							$tid = $db->insert_id( "INSERT INTO " . NV_PREFIXLANG . "_" . $module_data . "_tags (numnhadat, alias, description, image, keywords) VALUES (1, :alias, '', '', :keyword)", "tid", $array_insert );
						}
						else
						{
							if( $alias != $alias_i )
							...
							$db->query( 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_tags SET numnhadat = numnhadat+1 WHERE tid = ' . $tid );
						}
