		...
		else
		{
			...
			if( intval( $check_rows ) > 0 )
			{
				if( $delallcheckss == md5( $catid . session_id() . $global_config['sitekey'] ) )
				{
					...
					$catidnhadat = $nv_Request->get_int( 'catidnhadat', 'post', 0 );
					...
					elseif( ! empty( $delcatandrows ) )
					{
						...
						while( $row = $sql->fetch() )
						{
							...
							else
							{
								...
								$arr_catid_nhadat = array_diff( $arr_catid_old, $arr_catid_i );
								if( $catid == $row['catid'] )
								{
									$row['catid'] = $arr_catid_nhadat[0];
								}
								foreach( $arr_catid_nhadat as $catid_i )
								{
									$db->query( "UPDATE " . NV_PREFIXLANG . "_" . $module_data . "_" . $catid_i . " SET catid=" . $row['catid'] . ", listcatid = '" . implode( ',', $arr_catid_nhadat ) . "' WHERE id =" . $row['id'] );
								}
								$db->query( "UPDATE " . NV_PREFIXLANG . "_" . $module_data . "_rows SET catid=" . $row['catid'] . ", listcatid = '" . implode( ',', $arr_catid_nhadat ) . "' WHERE id =" . $row['id'] );
							}
						}
					...
					elseif( ! empty( $movecat ) and $catidnhadat > 0 and $catidnhadat != $catid )
					{
						list( $catidnhadat, $nhadattitle ) = $db->query( "SELECT catid, title FROM " . NV_PREFIXLANG . "_" . $module_data . "_cat WHERE catid =" . $catidnhadat )->fetch( 3 );
						if( $catidnhadat > 0 )
						{
							nv_insert_logs( NV_LANG_DATA, $module_name, $lang_module['move'], $title . ' --> ' . $nhadattitle, $admin_info['userid'] );
							...
							while( $row = $sql->fetch() )
							{
								...
								$arr_catid_i = array( $catid );
								$arr_catid_nhadat = array_diff( $arr_catid_old, $arr_catid_i );
								if( ! in_array( $catidnhadat, $arr_catid_nhadat ) )
								{
									$db->query( "INSERT INTO " . NV_PREFIXLANG . "_" . $module_data . "_" . $catidnhadat . " SELECT * FROM " . NV_PREFIXLANG . "_" . $module_data . "_rows WHERE id=" . $row['id'] );
									$arr_catid_nhadat[] = $catidnhadat;
								}
								if( $catid == $row['catid'] )
								{
									$row['catid'] = $catidnhadat;
								}
								foreach( $arr_catid_nhadat as $catid_i )
								{
									$db->query( "UPDATE " . NV_PREFIXLANG . "_" . $module_data . "_" . $catid_i . " SET catid=" . $row['catid'] . ", listcatid = '" . implode( ',', $arr_catid_nhadat ) . "' WHERE id =" . $row['id'] );
