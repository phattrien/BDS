...
if( nv_function_exists( "curl_init" ) and nv_function_exists( "curl_exec" ) )
{
	...
	if( $id > 0 )
	{
		...
		if( $nhadat_contents['id'] > 0 and ! empty( $prcservice ) )
		{
			if( $nhadat_contents['status'] == 1 and $nhadat_contents['publtime'] < NV_CURRENTTIME + 1 and ( $nhadat_contents['exptime'] == 0 or $nhadat_contents['exptime'] > NV_CURRENTTIME + 1 ) )
			{
				if( $nv_Request->get_string( 'checkss', 'post,get', '' ) == md5( $id . $global_config['sitekey'] . session_id() ) )
				{
					...
					if( empty( $getdata ) )
					{
						$page_title = $lang_module['rpc'] . ": " . $nhadat_contents['title'];
					...
					else
					{
						...
						$listcatid_arr = explode( ',', $nhadat_contents['listcatid'] );
						$catid_i = $listcatid_arr[0];

						$webtitle = htmlspecialchars( nv_unhtmlspecialchars( $nhadat_contents['title'] ), ENT_QUOTES );
						$webhome = nv_url_rewrite( NV_MY_DOMAIN . NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA );
						$linkpage = nv_url_rewrite( NV_MY_DOMAIN . NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $global_array_cat[$catid_i]['alias'] . '/' . $nhadat_contents['alias'] . '-' . $nhadat_contents['id'], 1 );
