...
if( isset( $key{NV_MIN_SEARCH_LENGTH - 1} ) )
{
	...
	elseif( $check_num == 4 )
	{
		$where = "AND (tb1.gia LIKE '%" . $dbkey . "%')";
	}
	elseif( $check_num == 5 )
	{
		$where = "AND (tb1.huong LIKE '%" . $dbkey . "%')";
	}
	elseif( $check_num == 6 )
	{
		$where = "AND (tb1.dientich LIKE '%" . $dbkey . "%')";
	}
	elseif( $check_num == 7 )
	{
		$where = "AND (tb1.lienhe LIKE '%" . $dbkey . "%')";
	}
	elseif( $check_num == 8 )
	{
		$where = "AND (tb1.vitri LIKE '%" . $dbkey . "%')";
	}
	...
	$db->select( 'tb1.id,tb1.title,tb1.alias,tb1.catid,tb1.hometext,tb1.author,tb1.publtime,tb1.homeimgfile, tb1.homeimgthumb,tb1.sourceid,tb1.lienhe,tb1.dientich,tb1.diachi,tb1.tel,tb1.gia,tb1.dvt,tb1.huong,tb1.vitri,tb1.ghichu' )
		->limit( $per_page )
	...
	while( list( $id, $title, $alias, $catid, $hometext, $author, $publtime, $homeimgfile, $homeimgthumb, $sourceid, $lienhe, $dientich, $diachi, $tel, $gia, $dvt, $huong, $vitri, $ghichu ) = $result->fetch( 3 ) )
	{
