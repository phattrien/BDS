...
if( ! defined( 'NV_IS_MOD_NHADAT' ) ) die( 'Stop!!!' );
...
if( $nv_Request->isset_request( 'contentid', 'get,post' ) and $fcheckss == $checkss )
{
	...
	if( $nv_Request->isset_request( 'contentid', 'post' ) )
	{
		...
		$rowcontent['gia'] = $nv_Request->get_title( 'gia', 'post', '', 1 );
		$rowcontent['dvt'] = $nv_Request->get_title( 'dvt', 'post',  '', 1 );
		$rowcontent['dientich'] = $nv_Request->get_title( 'dientich', 'post', '', 1 );
		$rowcontent['huong'] = $nv_Request->get_title( 'huong', 'post', '', 1 );
		$rowcontent['vitri'] = $nv_Request->get_title( 'vitri', 'post', '', 1 );
		$rowcontent['lienhe'] = $nv_Request->get_title( 'lienhe', 'post', '', 1 );
		$rowcontent['diachi'] = $nv_Request->get_title( 'diachi', 'post', '', 1 );
		$rowcontent['tel'] = $nv_Request->get_title( 'tel', 'post', '', 1 );
		$rowcontent['ghichu'] = $nv_Request->get_title( 'ghichu', 'post', '', 1 );
		...
		else
		{
			...
			if( $rowcontent['id'] == 0 )
			{
				$_sql = "INSERT INTO " . NV_PREFIXLANG . "_" . $module_data . "_rows
						(catid, listcatid, topicid, admin_id, author, gia, dvt, dientich, vitri,  huong,lienhe, diachi, tel, ghichu, sourceid, addtime, edittime, status, publtime, exptime, archive, title, alias, hometext, homeimgfile, homeimgalt, homeimgthumb, inhome, allowed_comm, allowed_rating, hitstotal, hitscm, total_rating, click_rating) VALUES
						 ...
						 " . $db->quote( $rowcontent['gia'] ) . ",
						 " . $db->quote( $rowcontent['dvt'] ) . ",
						 " . $db->quote( $rowcontent['dientich'] ) . ",
						 " . $db->quote( $rowcontent['vitri'] ) . ",
						 " . $db->quote( $rowcontent['huong'] ) . ",
						 " . $db->quote( $rowcontent['lienhe'] ) . ",
						 " . $db->quote( $rowcontent['diachi'] ) . ",
						 " . $db->quote( $rowcontent['tel'] ) . ",
						 " . $db->quote( $rowcontent['ghichu'] ) . ",
			...
			else
			{
				...
				$_sql = "UPDATE " . NV_PREFIXLANG . "_" . $module_data . "_rows SET
						 ...
						 gia=" . $db->quote( $rowcontent['gia'] ) . ",
						 dvt=" . $db->quote( $rowcontent['dvt'] ) . ",
						 dientich=" . $db->quote( $rowcontent['dientich'] ) . ",
						 huong=" . $db->quote( $rowcontent['huong'] ) . ",
						 vitri=" . $db->quote( $rowcontent['vitri'] ) . ",
						 lienhe=" . $db->quote( $rowcontent['lienhe'] ) . ",
						 diachi=" . $db->quote( $rowcontent['diachi'] ) . ",
						 tel=" . $db->quote( $rowcontent['tel'] ) . ",
						 ghichu=" . $db->quote( $rowcontent['ghichu'] ) . ",
			...
			// , lienhe, diachi, tel, gia, dvt, huong, vitri, ghichu, dientich	
			if( $db->exec( $_sql ) )
				{
	...
	if( $op == 'main' ) $xtpl->parse( 'main.logosite' );
	else  $xtpl->parse( 'main.no_logosite' );
...
elseif( defined( 'NV_IS_USER' ) )
{
	...
	$db->select( 'id, catid, listcatid, topicid, admin_id, author, gia, dvt, dientich, vitri,  huong, lienhe, diachi, tel, ghichu, sourceid, addtime, edittime, status, publtime, title, alias, hometext, homeimgfile, homeimgalt, homeimgthumb, allowed_rating, hitstotal, hitscm, total_rating, click_rating' )
		->order( 'id DESC' )
		->limit( $per_page )
