...
if( ! nv_function_exists( 'nv_nhadat_block_nhadat' ) )
{

	function nv_block_config_nhadat( $module, $data_block, $lang_block )
	{
		$html = '<tr>';
	...
	function nv_block_config_nhadat_submit( $module, $lang_block )
	{
		global $nv_Request;
		$return = array();
		$return['error'] = array();
	...
	function nv_nhadat_block_nhadat( $block_config, $mod_data )
	{
		...
		$cache_file = NV_LANG_DATA . '__block_nhadat_' . $numrow . '_' . NV_CACHE_PREFIX . '.cache';
		if( ( $cache = nv_get_cache( $module, $cache_file ) ) != false )
		{
			$array_block_nhadat = unserialize( $cache );
		}
		else
		{
			$array_block_nhadat = array();
			...
			$cache = serialize( $array_block_nhadat );
			nv_set_cache( $module, $cache_file, $cache );
		}

		if( file_exists( NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/nhadat/block_nhadat.tpl' ) )
		{
		...
		$xtpl = new XTemplate( 'block_nhadat.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/nhadat/' );
		
		foreach( $array_block_nhadat as $array_nhadat )
		{
			$array_nhadat['hometext'] = nv_clean60( $array_nhadat['hometext'], $block_config['tooltip_length'] );
			$xtpl->assign( 'blocknhadat', $array_nhadat );
			if( ! empty( $array_nhadat['imgurl'] ) )
			{
			...
			{
				$xtpl->assign( 'TITLE', 'title="' . $array_nhadat['title'] . '"' );
			}
...
if( defined( 'NV_SYSTEM' ) )
{
	...
	if( isset( $site_mods[$module] ) )
	{
		...
		$content = nv_nhadat_block_nhadat( $block_config, $mod_data );
	}
}