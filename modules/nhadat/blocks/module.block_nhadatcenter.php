...
if( ! nv_function_exists( 'nv_nhadat_block_nhadatcenter' ) )
{

	function nv_block_config_nhadat_nhadatcenter( $module, $data_block, $lang_block )
	{
		$html .= '<tr>';
	...
	function nv_block_config_nhadat_nhadatcenter_submit( $module, $lang_block )
	{
		global $nv_Request;
		$return = array();
		$return['error'] = array();
	...
	function nv_nhadat_block_nhadatcenter( $block_config )
	{
		global $module_data, $module_name, $module_file, $global_array_cat, $global_config, $lang_module, $db, $module_config, $module_info;
		
		$xtpl = new XTemplate( 'block_nhadatcenter.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file );
...
if( defined( 'NV_SYSTEM' ) )
{
	...
	$content = nv_nhadat_block_nhadatcenter( $block_config );
}