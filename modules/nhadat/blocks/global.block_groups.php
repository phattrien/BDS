...
if( ! nv_function_exists( 'nv_block_nhadat_groups' ) )
{
	function nv_block_config_nhadat_groups( $module, $data_block, $lang_block )
	{
		global $site_mods;
	...
	function nv_block_config_nhadat_groups_submit( $module, $lang_block )
	{
		global $nv_Request;
		$return = array();
		$return['error'] = array();
	...
	function nv_block_nhadat_groups( $block_config )
	{
		...
		if( ! empty( $list ) )
		{
			if( file_exists( NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/nhadat/block_groups.tpl' ) )
			{
			...
			$xtpl = new XTemplate( 'block_groups.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/nhadat' );
			foreach( $list as $l )
			{
...
if( defined( 'NV_SYSTEM' ) )
{
	...
	if( isset( $site_mods[$module] ) )
	{
		...
		$content = nv_block_nhadat_groups( $block_config );
	}
}
