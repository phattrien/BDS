...
if( ! nv_function_exists( 'nv_nhadat_category' ) )
{

	function nv_block_config_nhadat_category( $module, $data_block, $lang_block )
	{
	    global $site_mods;
		$html .= '<tr>';
	...
	function nv_block_config_nhadat_category_submit( $module, $lang_block )
	{
		global $nv_Request;
		$return = array();
		$return['error'] = array();
	...
	function nv_nhadat_category( $block_config )
	{
		global $module_array_cat, $module_info, $lang_module;

		$xtpl = new XTemplate( 'block_category.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/nhadat' );
			...
			foreach( $module_array_cat as $cat )
			{
				if( $block_config['catid'] == 0 && $cat['parentid'] == 0 || ( $block_config['catid'] > 0 && $cat['parentid'] == $block_config['catid'] ) )
				{
					...
					if( ! empty( $cat['subcatid'] ) )
					{
						...
						$html .= nv_nhadat_sub_category( $cat['subcatid'], $title_length );
					}
					$html .= "</li>\n";
				}
			}
	...
	function nv_nhadat_sub_category( $list_sub, $title_length )
	{
		...
		else
		{
			...
			foreach( $list as $catid )
			{
				...
				if( ! empty( $module_array_cat[$catid]['subcatid'] ) ) $html .= nv_nhadat_sub_category( $module_array_cat[$catid]['subcatid'], $title_length );
				$html .= "</li>\n";
			}
			$html .= "</ul>\n";
			return $html;
...
if( defined( 'NV_SYSTEM' ) )
{
	...
	if( isset( $site_mods[$module] ) )
	{
		...
		$content = nv_nhadat_category( $block_config );
	}
}