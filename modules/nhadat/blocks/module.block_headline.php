...
{
	function nv_block_config_nhadat_headline( $module, $data_block, $lang_block )
	{
		global $lang_module;
		$html .= '<tr>';
	...
	function nv_block_config_nhadat_headline_submit( $module, $lang_block )
	{
		global $nv_Request;
		$return = array();
		$return['error'] = array();
	...
	function nv_block_headline( $block_config )
	{
		...
		{
			$hot_nhadat = $array_bid_content[1]['content'];
			$a = 0;
			foreach( $hot_nhadat as $hot_nhadat_i )
			{
				if( ! empty( $hot_nhadat_i['homeimgfile'] ) and file_exists( NV_UPLOADS_REAL_DIR . '/' . $module_name . '/' . $hot_nhadat_i['homeimgfile'] ) )
				{
					$images_url = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_name . '/' . $hot_nhadat_i['homeimgfile'];
				}
				elseif( nv_is_url( $hot_nhadat_i['homeimgfile'] ) )
				{
					$images_url = $hot_nhadat_i['homeimgfile'];
				}

				if( ! empty( $images_url ) )
				{
					$hot_nhadat_i['image_alt'] = ! empty( $hot_nhadat_i['homeimgalt'] ) ? $hot_nhadat_i['homeimgalt'] : $hot_nhadat_i['title'];
					$hot_nhadat_i['imgID'] = $a;
					$images[] = $images_url;
					$xtpl->assign( 'HOTSNEWS', $hot_nhadat_i );
					$xtpl->parse( 'main.hots_nhadat_img.loop' );
					++$a;
				}
			}
			$xtpl->parse( 'main.hots_nhadat_img' );
		}
